...
public class Possible_list extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final List<Item> possibleArrayList = helper.getAllData3();
        final CustomAdapter adapter = new CustomAdapter(this, possibleArrayList);
       ...
       /*step:6 adapter call*/
        ...
        ListView myPossibleWishList = (ListView) findViewById(R.id.viewB_possible_list);
        myPossibleWishList.setAdapter(adapter);


        /* step:5 create array list*/
        final List<Item> PossibleArrayList = new ArrayList<Item>();
            ...
            public void onClick(View v) {
               ...
               /*step:4 Validating form*/
                ...
                if (!PossibleAmountInputET.getText().toString().equals("") ||
                    ...
                    PossibleArrayList.add(item);

                /*step:14 collection of  data show by database*/
                    item.setWishString("Item:" + PossibleArrayList.size());
                    item.setWishString(PossibleHeadInputET.getText().toString());

                    item.setAmountInteger("Item:" + PossibleArrayList.size());
        ...
        myPossibleWishList.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
            ...
            public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                helper.deleteData3(PossibleArrayList.get(position).getWishId());
                PossibleArrayList.remove(position);
                adapter.notifyDataSetChanged();

                /*adapterChanged();*/
                Toast.makeText(getBaseContext(), "One Possible Item Remove", Toast.LENGTH_SHORT).show();
