...
public class Yearly_list extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final List<Item> yearlyArrayList = helper.getAllData();
        final CustomAdapter adapter = new CustomAdapter(this, yearlyArrayList);
       ...
       /*step:6 adapter call*/
        ...
        final List<Item> YearlytArrayList = new ArrayList<Item>();
            ...
            public void onClick(View v) {
               ...
               /*step:4 Validating form*/
                ...
                if (!YearlyHeadInputET.getText().toString().equals("") ||
                    ...
                    yearlyArrayList.add(item);

                /*step:14 collection of  data show by database*/
                    item.setWishString("Item:" + YearlytArrayList.size());
                    item.setWishString(YearlyHeadInputET.getText().toString());

                    item.setAmountInteger("Item:" + YearlytArrayList.size());
                    item.setAmountInteger(YearlyAmountInputET.getText().toString());

                    DatabaseHelper helper = new DatabaseHelper(getBaseContext());
                    helper.insertData(inputTitle, inputAmount, dateString);
                    YearlytArrayList.add(item);
        ...
        myYearlyWishList.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
            @Override
            public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                helper.deleteData(yearlyArrayList.get(position).getWishId());
                yearlyArrayList.remove(position);
                adapter.notifyDataSetChanged();

                /*adapterChanged();*/
                Toast.makeText(getBaseContext(), "One Yearly Item Remove", Toast.LENGTH_SHORT).show();
